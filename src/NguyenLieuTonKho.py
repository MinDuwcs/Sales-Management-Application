...
def add_NguyenLieu(TEN_NGUYENLIEU, DON_VI_TINH, NGAY_CAPNHAT, GHI_CHU,so_luong = 0):
    if conn and cursor:
        query = '''
            INSERT INTO NGUYENLIEU_TONKHO (TEN_NGUYENLIEU, DON_VI_TINH,SO_LUONG_TON, NGAY_CAPNHAT, GHI_CHU)
            VALUES (?, ?, ?, ?,?)
        '''
        try:
            cursor.execute(query, (TEN_NGUYENLIEU, DON_VI_TINH,so_luong, NGAY_CAPNHAT, GHI_CHU))
            conn.commit()
