...
# Kết nối SQL Server – thông tin đọc từ file .env
...
class UserManager:
    ...
    def get_user_id_from_username(self, username):
        try:
            self.cursor.execute("SELECT ID_NHANSU FROM ACCOUNT WHERE Username = ?", (username,))
            ...
            QMessageBox.critical(None, "Lỗi", f"Lỗi khi lấy ID nhân viên: {e}")
            return None
    ...
    def register(self, username, password, email, id_nhansu):
        if not username or not password or not email or not id_nhansu:
        ...
        try:
            ...
            self.cursor.execute("SELECT TEN_NHANSU FROM dbo.NHAN_SU where ID_NHANSU = ?",(id_nhansu))
            if self.cursor.fetchone() is None:
                return False, "id này không tồn tại"
            self.cursor.execute("SELECT Email FROM dbo.ACCOUNT WHERE Email = ?", (email,))
            if self.cursor.fetchone():
                return False, "Email đã được sử dụng"
            self.cursor.execute(
                "INSERT INTO dbo.ACCOUNT (Username, Password, Email,ID_NHANSU) VALUES (?, ?, ?,?)",
                (username, password, email,id_nhansu)
            )
            self.conn.commit()
...
class LoginWindow(QMainWindow):
    ...
    def login(self):
        ...
        if not username or not password:
            QMessageBox.warning(self, "Lỗi", "Điền đủ thông tin!")
            return

        if self.user_manager.authenticate(username, password):
            logged_in_user_id = self.user_manager.get_user_id_from_username(username) # Lấy ID ở đây
            QMessageBox.information(self, "Thành công", "Đăng nhập thành công!")
            self.main_window = CoffeePOS(self.conn, logged_in_user_id)
            self.main_window.show()
            self.close()
        else:
            QMessageBox.critical(self, "Lỗi", "Sai thông tin đăng nhập!")
    ...
    def signup(self):
        ...
        ID_NHANSU_label = QLabel("ID:")
        ID_NHANSU_label.setStyleSheet("color: white; font-size: 14px;")
        ID_NHANSU_input = QLineEdit()
        ID_NHANSU_input.setStyleSheet("background-color: white; border-radius: 5px; color: black; font-size: 14px; padding: 5px;")
        ...
        form_layout.addWidget(ID_NHANSU_label, 3 ,0)
        form_layout.addWidget(ID_NHANSU_input,3,1)
        ...
        """)
        confirm_button.clicked.connect(lambda: self.handle_signup(dialog, username_input.text(), password_input.text(), email_input.text(),ID_NHANSU_input.text() ))
    ...
    # Xử lý đăng ký
    def handle_signup(self, dialog, username, password, email,id_nhansu):

        success, message = self.user_manager.register(username.strip(), password.strip(), email.strip(),id_nhansu.strip())
        if success:
...
class CoffeePOS(QMainWindow):
    def __init__(self,connection,logged_in_user_id):
        super().__init__()
        self.conn = connection
        self.cursor = connection.cursor()
    ...
    def mo_quan_ly_nguyen_lieu(self):
        if self.logged_in_user_id: # Kiểm tra xem đã có ID_NHANSU chưa
            self.nl_window = NguyenLieuTonKhoUI(self.conn, self.logged_in_user_id) # Truyền ID_NHANSU
            self.nl_window.show()
        else:
            QMessageBox.warning(self, "Chưa đăng nhập", "Bạn chưa đăng nhập")
    ...
    def setup_ui(self, main_layout):
        ...
        search_input_layout.addWidget(refresh_btn)
    ...
    def refresh_menu(self):
        # Cập nhật menu_items từ database
        def get_menu():
            conn = pyodbc.connect(CONNECTION_STRING)
            cursor = conn.cursor()
            cursor.execute("SELECT TEN_MON, DON_GIA FROM dbo.MON")
    ...
    def show_print_receipt_dialog(self):
        ...
        def update_receipt():
            ...
            max_name_len = max(len("Món"), max(len(item[:27]) for item in self.current_order.keys()))
            max_qty_len = max(len("SL"), max(len(str(data['quantity'])) for data in self.current_order.values()))
            max_price_len = max(len("Giá"), max(len(f"{data['price']:,.0f} VND") for data in self.current_order.values()))

            # Tiêu đề cột
            receipt_content += f"{'Món':<{max_name_len + 2}}{'SL':^{max_qty_len + 2}}{'Giá':>{max_price_len}}\n"
            receipt_content += "-" * (max_name_len + max_qty_len + max_price_len + 4) + "\n"

            # Nội dung các món
            for item, data in self.current_order.items():
                name = item[:max_name_len].ljust(max_name_len + 2)  # Căn trái với khoảng cách
                qty = str(data['quantity']).center(max_qty_len + 2)  # Căn giữa
                price = f"{data['price']:,.0f} VND".rjust(max_price_len)  # Căn phải
                receipt_content += f"{name}{qty}{price}\n"

            # Tổng cộng
            receipt_content += "-" * 60 + "\n"
            receipt_content += f"{'Tổng cộng':<36}{f'{self.total_amount:>22,.0f} VNĐ':>26}\n"  # Tăng độ rộng tổng cộng
            receipt_content += "\nCảm ơn quý khách!\n"

            # Đảm bảo font monospace
            self.receipt_text.setStyleSheet("background-color: white; color: black; font-size: 14px; font-family: 'Courier New', monospace; white-space: pre;")
    ...
    def show_notification_dialog(self):
        ...
        def clear_and_refresh():
            self.clear_notifications()       # xóa dữ liệu
            notification_list.clear()        # xóa giao diện
    ...
    def show_staff_management(self):
        ...
        self.cursor.execute("SELECT ID_NHANSU,TEN_NHANSU, VAI_TRO, ID_NG_QUANLY FROM dbo.NHAN_SU")
        rows = self.cursor.fetchall()

        # Format dữ liệu thành chuỗi để hiển thị
        staff_data = [f"{id_nv} - {ten} - ({chucvu}) - id người quản lý {id_ql}" for id_nv,ten, chucvu, id_ql in rows]
    ...
    def add_staff(self, dialog, staff_list):
        ...
        id_ng_ql, ok3 = QInputDialog.getText(dialog, "ID người quản lý", " Nhập ID người quản lý:")
        if not ok3 or not id_ng_ql:
            QMessageBox.warning(self, "thiếu thông tin","Bạn phải nhập id người quản lý")
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO NHAN_SU (ID_NHANSU, TEN_NHANSU, VAI_TRO, ID_NG_QUANLY) VALUES (?, ?, ?, ?)",
                (new_id, ten_nv, vai_tro, id_ng_ql)
            )
            self.conn.commit()

            staff_list.addItem(f"{new_id} - {ten_nv} ({vai_tro}) - id_ql{id_ng_ql}")
...
class NguyenLieuTonKhoUI(QWidget):
    def __init__(self,connection,logged_in_user_id):
        ...
        self.conn =connection
        self.cursor = connection.cursor()
        self.logged_in_user_id = logged_in_user_id # Lưu ID ở đây
    ...
    def cap_nhat_so_luong(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Sai định dạng", "Nhập số lượng là số.")
            return
        id_nl_text = self.table.item(selected, 0).text()
        ten_nl = self.table.item(selected, 1).text()  # Lấy tên nguyên liệu để log
        dv_nl = self.table.item(selected, 2).text()  # Lấy đơn vị tính để log
        so_luong_hien_tai_item = self.table.item(selected, 3) # Lấy item
        if so_luong_hien_tai_item is not None and so_luong_hien_tai_item.text():
            try:
                so_luong_hien_tai = float(so_luong_hien_tai_item.text())  # Lấy số lượng hiện tại
            except ValueError:
                QMessageBox.warning(self, "Lỗi", "Giá trị số lượng hiện tại không hợp lệ.")
                return
        else:
            QMessageBox.warning(self, "Lỗi", "Giá trị số lượng hiện tại không tồn tại.")
            return
        id_nl = int(id_nl_text)
        cap_nhat_ton_kho(id_nl, delta)
        # Ghi lại thay đổi vào bảng THAY_DOI_NGUYENLIEU
        trang_thai = "Nhập" if delta > 0 else "Xuất" if delta < 0 else "Không đổi"
        self.ghi_log_thay_doi(id_nl, ten_nl, self.logged_in_user_id, trang_thai, dv_nl, delta, datetime.today())
        self.so_luong_input.clear()
        self.tai_lai_bang()
    ...
    def ghi_log_thay_doi(self, id_nguyenlieu, ten_nguyenlieu, id_nhansu, trang_thai, don_vi_tinh, so_luong_thay_doi, ngay_capnhat):
        try:
            self.cursor.execute(
                "INSERT INTO THAY_DOI_NGUYENLIEU (ID_NGUYENLIEU, ID_NHANSU, TEN_NGUYENLIEU, TRANG_THAI, DON_VI_TINH, SO_LUONG_THAY_DOI, NGAY_CAPNHAT) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (id_nguyenlieu, id_nhansu, ten_nguyenlieu, trang_thai, don_vi_tinh, so_luong_thay_doi, ngay_capnhat),
            )
            self.conn.commit()
        except Exception as e:
            QMessageBox.critical(self, "Lỗi", f"Lỗi ghi log: {e}")
            self.conn.rollback()
